...
const events = [
  ...
  { id: 2, title: 'Reunión Vecinal Barrio Centro', date: '18 de febrero', time: '19:00 hs.', location: 'Sede Vecinal', category: 'Comunidad', description: 'Espacio de diálogo entre vecinos y autoridades municipales.' },
]
    ...
    <section id="calendario" className="section-modern bg-slate-50">
      <div className="container-modern">
        <div className="text-center mb-12">
          <span className="badge-modern bg-muni-100 text-muni-700 mb-4">Agenda Municipal</span>
          <h2 className="font-display text-3xl md:text-4xl font-bold text-slate-900 mb-4">Calendario de eventos</h2>
        </div>

        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-white rounded-2xl shadow-soft p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold flex items-center gap-2 text-slate-900"><CalendarIcon className="h-5 w-5 text-muni-500" />Calendario</h3>
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="ghost" size="sm" className="text-muni-600">Ver completo</Button>
                </PopoverTrigger>
            ...
            <Calendar mode="single" selected={date} onSelect={setDate} className="rounded-xl border-0" />
          </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {events.map((event) => (
                <div key={event.id} className="group bg-white rounded-2xl p-5 shadow-soft">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                    ...
                    <div className="flex-1">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${categoryColors[event.category]}`}>{event.category}</span>
                      <h4 className="text-lg font-semibold text-slate-900 mb-1 mt-2">{event.title}</h4>
